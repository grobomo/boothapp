...
async function runTests() {
  ...
  // Retries then succeeds
  {
    ...
    console.log('  [PASS] retries then succeeds on 3rd call');
  }
  ...
  // Delays increase exponentially
  {
    ...
    assert.ok(delays[1] > delays[0], `delay[1]=${delays[1]} > delay[0]=${delays[0]}`);
    assert.ok(delays[2] > delays[1], `delay[2]=${delays[2]} > delay[1]=${delays[1]}`);
  ...
  // Respects maxDelayMs cap
  {
    ...
    for (const d of delays) {
      assert.ok(d <= 15000, `delay ${d} <= maxDelayMs 15000`);
    }
    console.log('  [PASS] respects maxDelayMs cap');
  }

  // maxRetries=0 means single attempt
  {
    let calls = 0;
    try {
